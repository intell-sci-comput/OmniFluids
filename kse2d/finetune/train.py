...
def train(config, net):
    ...
    train_param = torch.load(config.data_path+f'dataset/param_train{data_name}')[:num_train].float().to(device)
    print(train_data.shape)
    test_data = torch.load(config.data_path+f'dataset/test{data_name}')[:, ::sub, ::sub, ...].float().to(device)
    test_param = torch.load(config.data_path+f'dataset/param_test{data_name}').float().to(device)
    print(test_data.shape)
    val_data = torch.load(config.data_path+f'dataset/val{data_name}')[:, ::sub, ::sub, ...].float().to(device)
    val_param = torch.load(config.data_path+f'dataset/param_val{data_name}').float().to(device)
...
#     num_train = config.num_train
#     rollout_DT = config.rollout_DT  
#     train_data_ratio = train_data_dict['record_ratio']
#     sub_t = round(train_data_ratio / (1.0/rollout_DT))
#     train_total_iter = round(train_data_dict['T'] / rollout_DT)

#     train_data = torch.load(config.data_path+f'dataset/data_train{data_name}')[:num_train, ::sub, ::sub, ::sub_t].float().to(device)
#     train_param = torch.load(config.data_path+f'dataset/f_train{data_name}')[:num_train,::sub, ::sub, :].float().to(device)
#     print(train_data.shape)
#     test_data = torch.load(config.data_path+f'dataset/data_test{data_name}')[:, ::sub, ::sub, ...].float().to(device)
#     test_param = torch.load(config.data_path+f'dataset/f_test{data_name}')[:, ::sub, ::sub, ...].float().to(device)
#     print(test_data.shape)
#     val_data = torch.load(config.data_path+f'dataset/data_val{data_name}')[:, ::sub, ::sub, ...].float().to(device)
#     val_param = torch.load(config.data_path+f'dataset/f_val{data_name}')[:, ::sub, ::sub, ...].float().to(device)
#     print(val_data.shape)
...
#     for step in range(config.num_iterations):
#         net.train()
#         w_pre = train_data[..., 0:1]
#         loss = torch.zeros(1, device=device)
#         for t in range(train_total_iter):
#             w_pre = net(w_pre, train_param)[..., -1:]
#             w_gth = train_data[..., t+1:t+2]
#             loss = loss + torch.square(w_pre - w_gth).mean().sqrt()
#         loss.backward()
#         optimizer.step()
#         optimizer.zero_grad(set_to_none=True)
#         if step % 10 == 0:
