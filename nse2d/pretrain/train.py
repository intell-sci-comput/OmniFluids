...
def val(config, net, w_0, val_param):
    ...
    nu_test = val_param[:, 0, 0, 1]
    physics_loss = PSM_loss(w_pre, val_param[..., 0:1], 10**nu_test, config.rollout_DT, config.loss_mode)
...
def train(config, net):
    ...
    lognu_min, lognu_max = math.log10(test_data_dict['re'][0]), math.log10(test_data_dict['re'][1])
    data_size = test_data_dict['s'] // test_data_dict['sub']
    max_frequency = test_data_dict['max_frequency']
    batch_size = config.batch_size
    amplitude_range = (-test_data_dict['amplitude'], test_data_dict['amplitude'])
    size = config.size
    ...
    F_Sampler = Force_generation(size, max_frequency=max_frequency, amplitude_range=amplitude_range, device=device)
    val_size = config.val_size
    w0_val, f_val = GRF(val_size)[..., None], F_Sampler(val_size)[..., None]
    nu_val = - lognu_min - (lognu_max - lognu_min) * torch.rand(val_size).to(device)
    val_param = torch.concat([f_val, nu_val.reshape(-1, 1, 1, 1)*torch.ones_like(f_val)], dim=-1)
    ...
    for step in range(num_iterations+1):
        net.train()
        w0_train, f_train = GRF(batch_size, )[..., None], F_Sampler(batch_size)[..., None]
        nu_train = - lognu_min - (lognu_max - lognu_min) * torch.rand(batch_size).to(device)
        param = torch.concat([f_train, nu_train.reshape(-1, 1, 1, 1)*torch.ones_like(f_train)], dim=-1)
